...
def resolve_helmet_model_path() -> str:
    candidates = [
        Path(ViT_MODEL_PATH),
...
def init_detectors() -> None:
    ...
    if helmet_detector is None:
        helmet_detector = HelmetDetector(
            model_path=resolve_helmet_model_path(),
...
def build_summary(persons: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def annotate_image(image_rgb: np.ndarray, persons: List[Dict[str, Any]]) -> np.ndarray:
...
async def detect_image(file: UploadFile) -> Dict[str, Any]:
    init_detectors()
    ...
    summary = build_summary(persons)
    annotated = annotate_image(image_rgb, persons)
...
async def detect_frame(file: UploadFile) -> Dict[str, Any]:
    init_detectors()
